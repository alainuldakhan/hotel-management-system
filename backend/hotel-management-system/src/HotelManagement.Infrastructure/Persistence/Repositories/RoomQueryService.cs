        ...
        Guid? roomTypeId = null, CancellationToken ct = default)
    {
        var sql = """
            SELECT
                ...
                r.number            AS Number,
                r.floor             AS Floor,
                CASE r.status WHEN 1 THEN 'Available' WHEN 2 THEN 'Occupied' WHEN 3 THEN 'Cleaning' WHEN 4 THEN 'Maintenance' WHEN 5 THEN 'OutOfService' ELSE r.status::text END AS Status,
